// W9 browser gate: real Online Match AI fill plus responsive mobile/desktop geometry.
const { spawn } = require('node:child_process');
const fs = require('node:fs');
const os = require('node:os');
const path = require('node:path');

const FRONT = process.env.SRSZQ_W9_FRONT_URL ?? 'http://127.0.0.1:5180/';
const API = process.env.SRSZQ_W9_API_URL ?? 'http://127.0.0.1:8090';
const DEBUG_PORT = 9400 + (process.pid % 200);
const BROWSER = process.env.BROWSER_PATH
  || (fs.existsSync('C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe')
    ? 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe'
    : 'C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe');
const OUT = path.join(__dirname, 'results', 'w9');
...
async function registerUser() {
  ...
  const response = await fetch(`${API}/api/register`, {
    ...
    body: JSON.stringify({ email: `w9-${suffix}@test.com`, username: `W9${suffix.slice(-8)}`, password: 'secret1' }),
  });
...
async function main() {
  ...
  const profile = path.join(os.tmpdir(), `srszq-w9-${process.pid}`);
  ...
  console.log(failures ? `\nW9 E2E: ${failures} FAILURE(S)` : '\nW9 E2E: ALL PASS');
  process.exit(failures ? 1 : 0);
}

main().catch((error) => {
  console.error('W9 E2E ERROR:', error);
  process.exit(1);
});
