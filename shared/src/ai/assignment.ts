...
/** 教程单个 AI 难度：仅 1★/2★/3★ 均匀随机，独立可重复。 */
export function tutorialAiLevel(rand: Rng = defaultRng): AiDifficulty {
  return pickUniform<AiDifficulty>([1, 2, 3], rand);
}
 ...
 */
export function pickOnlineSingleHumanAiDifficulty(rand: Rng = defaultRng): AiDifficulty {
  const r = rand();
  if (r < 0.2) return 2;
  if (r < 0.5) return 3;
  if (r < 0.9) return 4;
...
export function pickOnlineTwoHumanAiDifficulty(rand: Rng = defaultRng): AiDifficulty {
  return rand() < 0.6 ? 4 : 5;
}
