...
import { LEVEL_CONFIG, OFFLINE_LEVEL_CONFIG } from './config/defaultWeights';
...
/** LEVEL 4 — 3-PLY：固定深度 3 的 MaxN 搜索（覆盖完整三人行动循环） */
export function threePlySearch(state: GameState, opts: SearchAgentOpts = {}): SearchResult {
  const cfg = (opts.offline ? OFFLINE_LEVEL_CONFIG : LEVEL_CONFIG)['3ply'];
  return maxNSearch(
    state,
    {
...
/**
 * LEVEL 5 — MAXN：宽候选 MaxN（k ≥ 3ply）+ 换位表 + 叶节点必胜稳定化。
...
export function maxnSearch(state: GameState, opts: SearchAgentOpts = {}): SearchResult {
  const cfg = (opts.offline ? OFFLINE_LEVEL_CONFIG : LEVEL_CONFIG).maxn;
  return maxNSearch(
    state,
    {
      timeBudgetMs: opts.timeBudgetMs ?? cfg.timeBudgetMs,
